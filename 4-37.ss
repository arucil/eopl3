...
(define (apply-procedure-result p refs)
  (cases proc p
    [procedure (vars body e)
               (if (not (= (length vars) (length refs)))
                   (eopl:error 'apply-procedure
                               "argument number mismatch, expects ~A, got ~A"
                               (length vars)
                               (length refs))
                   (letrec ([rec (lambda (var-list ref-list)
                                   (if (null? var-list)
                                       e
                                       (extend-env (car var-list)
                                                   (newref (deref (car ref-list)))
                                                   (rec (cdr var-list)
                                                     (cdr ref-list)))))]
                            [new-env (rec vars refs)]
                            [ret (value-of body new-env)]
                            [copy-result (lambda (var-list ref-list)
                                           (unless (null? var-list)
                                             (setref! (car ref-list)
                                                      (deref (apply-env new-env
                                                                        (car var-list))))
                                             (copy-result (cdr var-list)
                                                          (cdr ref-list))))])
                     (copy-result vars refs)
                     ret))]))
...
(define (array-ref a index)
  (cases array a
    [an-array (ref size)
              (if (< -1 index size)
                  (deref (+ ref index))
                  (eopl:error 'array-ref "array index out of bound ~A (size ~A)" index size))]))

(define (array-get-ref a index)
  (cases array a
    [an-array (ref size)
              (if (< -1 index size)
                  (+ ref index)
                  (eopl:error 'array-get-ref "array index out of bound ~A (size ~A)" index size))]))
...
in let f = proc(z)
   ...
   in begin
        % call-by-value-result,  x not changed
        print(@(f x));
        print(@(f x));
        print(x);

        % call-by-reference, x increased
        print((f x));
        print((f x));
        print(x)
      end
