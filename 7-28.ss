...
(define (subst-var exp var0 exp0)
  (let rec ([exp exp])
    (cases expression exp
      [const-exp (n) exp]
      [var-exp (var1)
               (if (eqv? var1 var0) exp0 exp)]
      [diff-exp (exp1 exp2)
                (diff-exp (rec exp1)
                          (rec exp2))]
      [zero?-exp (exp1)
      ...
      [proc-exp (vars otypes body)
                (if (exists? var0 vars)
                    exp
                    (proc-exp vars otypes
                              (rec body)))]
      [call-exp (exp1 exps)
                (call-exp (rec exp1)
                          (map rec exps))]
      [letrec-exp (ret-tys names varss arg-tyss bodies exp1)
                  (if (exists? var0 names)
                      exp
                      (let loop ([varss1 varss]
                                 [bodies bodies]
                                 [new-bodies '()])
                        (if (null? varss1)
                            (letrec-exp ret-tys names varss arg-tyss
                                        (reverse new-bodies)
                                        (rec exp1))
                            (loop (cdr varss1)
                                  (cdr bodies)
                   ...
                   (cases expression exp
                     ...
                     [let-exp (vars exps body)
                              ...
                              (let loop ([vars vars] [exps exps])
                                (unless (null? vars)
                                  (eopl:printf "~A~A = " (indent ind) (car vars))
                                  (disp (car exps) (indent ind))
                                  (loop (cdr vars) (cdr exps))))
                              (eopl:printf "~%~Ain~%" ind)
                              (disp body (indent ind))]
                     [letrec-exp (ret-tys p-names b-varss arg-tyss p-bodies body)
