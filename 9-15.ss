...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (class-decl
     ("class" identifier "extends" identifier (arbno static-decl) (arbno "field" identifier) (arbno method-decl))
     a-class-decl)
...
(define (init-class-env! c-decls)
  (set! the-class-env (list (cons 'object (a-class #f '() '() (empty-env)))))
                  ...
                  (let ([f-names (append-field-names (class->field-names (lookup-class s-name))
                    ...
                    (add-to-class-env! c-name
                                       (a-class s-name f-names
                                                ...
                                                (static-decl->static-env
                                                 s-decls
                                                 (class->static-env (lookup-class s-name))))))]))

(define (static-decl->static-env s-decls s-env0)
  (if (null? s-decls)
      s-env0
      (cases static-decl (car s-decls)
        [a-static-decl (name exp1)
                       (extend-env name
                                   (newref (value-of exp1 (empty-env)))
                                   (static-decl->static-env (cdr s-decls) s-env0))])))
...
(define (class->field-names c)
  (cases class c
    [a-class (s-name f-names m-env se)
             f-names]))
...
class foo extends object
 static tally1 = 0
 static tally2 = new tally()
 field id
 method initialize()
  begin
   set tally1 = +(tally1, 10);
   set id = +(tally1, send tally2 count());
  end
 method count() send tally2 count()
 method get() list(tally1, send tally2 tally())
 method id() id
let o1 = new foo()
    o2 = new foo()
in
