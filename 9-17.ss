...
(define the-grammar
  '((program ((arbno "class" class-decl) expression) a-program)

    (class-decl
     (identifier "extends" identifier (arbno "field" identifier) (arbno method-decl))
     a-class-decl)
    ...
    (expression
     ("letclass" class-decl "in" expression)
     letclass-exp)
...
(define (lookup-class name cenv)
  (let ([p (assq name cenv)])
    (if p
        (cdr p)
...
(define (init-class-env c-decls)
    (let loop ([c-decls c-decls]
               [cenv (list (cons 'object (a-class #f '() '())))])
      (if (null? c-decls)
          cenv
          (loop (cdr c-decls)
                (add-class-to-class-env (car c-decls) cenv)))))

(define (add-class-to-class-env c-decl cenv)
  (cases class-decl c-decl
    [a-class-decl (c-name s-name f-names m-decls)
                  (let ([f-names (append-field-names (class->field-names (lookup-class s-name cenv))
                                                     f-names)])
                    (cons
                     (cons c-name
                           (a-class s-name f-names
                                    (merge-method-envs
                                     (class->method-env (lookup-class s-name cenv))
...
(define (find-method c-name name cenv)
  (let ([p (assq name (class->method-env (lookup-class c-name cenv)))])
    (if p
        (cdr p)
...
(define (apply-procedure p refs cenv)
  (cases proc p
...
(define (value-of exp env cenv)
  (cases expression exp
    [const-exp (num)
      (num-val num)]
    [diff-exp (exp1 exp2)
      (num-val (- (expval->num (value-of exp1 env cenv))
                  (expval->num (value-of exp2 env cenv))))]
    [plus-exp (exp1 exp2)
      (num-val (+ (expval->num (value-of exp1 env cenv))
                  (expval->num (value-of exp2 env cenv))))]
    [var-exp (var)
    ...
    [zero?-exp (exp1)
      (bool-val (zero? (expval->num (value-of exp1 env cenv))))]
    [if-exp (exp1 exp2 exp3)
      (if (expval->bool (value-of exp1 env cenv))
          (value-of exp2 env cenv)
          (value-of exp3 env cenv))]
    [let-exp (vars exps body)
             (value-of body (extend-env* vars
                                        (map newref (values-of-exps exps env cenv))
                                        env)
                       cenv)]
    [letrec-exp (p-names b-varss bodies exp)
                (value-of exp (extend-env-rec p-names b-varss bodies env) cenv)]
    ...
    [call-exp (exp1 exps)
              (apply-procedure (expval->proc (value-of exp1 env cenv))
                               (map newref (values-of-exps exps env cenv))
                               cenv)]

    [assign-exp (var exp1)
                (setref! (apply-env env var)
                         (value-of exp1 env cenv))]

    [begin-exp (exp1 exps)
               (let loop ([val (value-of exp1 env cenv)]
                          [exps exps])
                 (if (null? exps)
                     val
                     (loop (value-of (car exps) env cenv)
                           (cdr exps))))]

    [print-exp (exp1)
               (let rec ([val (value-of exp1 env cenv)]
                         [need-par #t])
                 (cases expval val
                   [num-val (num)
                            (display num)]
    ...
    [null-exp ()
              (null-val)]
    [null?-exp (exp1)
               (bool-val (expval-null? (value-of exp1 env cenv)))]
    [car-exp (exp1)
             (car (expval->pair (value-of exp1 env cenv)))]
    [cdr-exp (exp1)
             (cdr (expval->pair (value-of exp1 env cenv)))]
    [cons-exp (exp1 exp2)
              (pair-val (cons (value-of exp1 env cenv)
                              (value-of exp2 env cenv)))]
    [list-exp (exps)
              (make-list (values-of-exps exps env cenv))]

    [self-exp ()
              (apply-env env '%self)]
    [method-call-exp (obj-exp m-name rands)
                     (let ([obj (value-of obj-exp env cenv)])
                       (apply-method
                        (find-method (object->class-name obj)
                                     m-name cenv)
                        obj
                        (values-of-exps rands env cenv)
                        cenv))]
    [super-call-exp (m-name rands)
                    (apply-method (find-method (apply-env env '%super) m-name cenv)
                                  (apply-env env '%self)
                                  (values-of-exps rands env cenv)
                                  cenv)]
    [new-object-exp (c-name rands)
                    (let ([obj (new-object c-name cenv)])
                      (apply-method (find-method c-name 'initialize cenv)
                                    obj
                                    (values-of-exps rands env cenv)
                                    cenv)
                      obj)]
    [letclass-exp (c-decl exp1)
                  (value-of exp1 env
                            (add-class-to-class-env c-decl cenv))]
    ))
...
(define (values-of-exps exps env cenv)
  (if (null? exps)
      '()
      (cons (value-of (car exps) env cenv)
            (values-of-exps (cdr exps) env cenv))))

(define (new-object c-name cenv)
  (an-object c-name
             (map (lambda (f-name)
                    (newref (list 'uninitialized f-name)))
                  (class->field-names (lookup-class c-name cenv)))))

(define (apply-method m self args cenv)
  (cases method m
