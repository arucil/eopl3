...
(define the-cps-grammar
  '((cps-program (cps-tf-exp) cps-a-program)
    ...
    (cps-simple-exp
     ("cons" "(" cps-simple-exp "," cps-simple-exp ")")
     cps-cons-exp)
    ...
    (cps-simple-exp
     ("list" "(" (separated-list cps-simple-exp ",") ")")
     cps-list-exp)
    ...
    (cps-tf-exp
     ("let" (arbno identifier "=" cps-simple-exp) "in" cps-tf-exp)
     cps-let-exp)
...
(define (value-of-simple-exp exp env)
  (cases cps-simple-exp exp
    ...
    [cps-cons-exp (simple1 simple2)
                  (pair-val (cons (value-of-simple-exp simple1 env)
                                  (value-of-simple-exp simple2 env)))]
    [cps-car-exp (simple1)
                 (car (expval->pair (value-of-simple-exp simple1 env)))]
    [cps-cdr-exp (simple1)
                 (cdr (expval->pair (value-of-simple-exp simple1 env)))]
    [cps-list-exp (simples)
                  (let rec ([ls simples])
                    (if (null? ls)
                        (null-val)
                        (pair-val (cons (value-of-simple-exp (car ls)
                                                             env)
                                        (rec (cdr ls))))))]
    ))
