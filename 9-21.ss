...
(define (init-class-env! c-decls)
  (set! the-class-env (list (cons 'object (a-class #f '() (empty-method-env)))))
...
(define (find-method c-name m-name)
  (let rec ([m-env (class->method-env (lookup-class c-name))])
    (cases method-env m-env
      [method-env-node (mn m l r)
                       (cond
                         [(symbol<? m-name mn)
                          (rec l)]
                         [(symbol<? mn m-name)
  ...
  (if (null? m-decls)
      ...
      (cases method-decl (car m-decls)
        [a-method-decl (m-name vars body)
                       (expand-method-env m-name
                                          (a-method vars body s-name f-names)
                                          (method-decls->method-env (cdr m-decls)
                                                                    s-name
                                                                    f-names))])))

(define (merge-method-envs s-m-env new-m-env)
  (cases method-env new-m-env
    [method-env-node (mn m l r)
                     (merge-method-envs
                      (expand-method-env
                       mn m
                       (merge-method-envs s-m-env l))
                      r)]
    [method-env-leaf ()
                     s-m-env]))
...
(define (expand-method-env m-name method m-env)
  (cases method-env m-env
    [method-env-node (mn m l r)
                     (cond
                       [(symbol<? m-name mn)
                        (method-env-node
                         mn
                         m
                         (expand-method-env m-name method l)
                         r)]
                       [(symbol<? mn m-name)
                        (method-env-node
                         mn
                         m
                         l
                         (expand-method-env m-name method r))]
                       [else
                        (method-env-node m-name method l r)])]
    [method-env-leaf ()
                     (method-env-node m-name method
                                      (method-env-leaf)
                                      (method-env-leaf))]))
...
(define-datatype method-env method-env?
  [method-env-node
   (m-name identifier?)
   (method method?)
   (left method-env?)
...
(define empty-method-env method-env-leaf)
