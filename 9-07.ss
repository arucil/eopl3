              ...
              (value-of body
                        (extend-env*
                         ...
                         (extend-env-with-self-and-super
                          self
                          super-name
                          (build-env-for-method field-names
                                                (object->fields self)))))]))

(define (build-env-for-method field-names refs)
  (cond
    [(null? field-names)
     (empty-env)]
    [(memv #\% (string->list (symbol->string (car field-names))))
     (build-env-for-method (cdr field-names)
                           (cdr refs))]
    [else
     (extend-env (car field-names)
                 (car refs)
                 (build-env-for-method (cdr field-names)
                                       (cdr refs)))]))
