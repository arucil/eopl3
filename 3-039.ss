...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression
     ("%unpack" number #| list-length |# expression "in" expression)
     nl-unpack-exp)
  ...
  (cases expression exp
    ...
    [call-exp (exp1 exp2)
              (call-exp (translation-of exp1 senv)
                        (translation-of exp2 senv))]
    [unpack-exp (var-list exp body)
                (let loop ([vars var-list]
                           [se senv]
                           [n 0])
                  (if (null? vars)
                      (nl-unpack-exp n
                                     (translation-of exp senv)
                                     (translation-of body se))
                      (loop (cdr vars)
                            (extend-senv (car vars) se)
                            (+ n 1))))]
    [else
  ...
  (cases expression exp
    ...
    [print-exp (exp)
               (letrec ([rec (lambda (val need-par)
                 ...
                 (newline)
                 (num-val 1))]
    [nl-unpack-exp (n exp body)
                   (let loop ([list (value-of exp env)]
                              [e env]
                              [n n])
                     (if (expval-null? list)
                         (if (zero? n)
                             (value-of body e)
                             (eopl:error 'value-of "unpack operand number mismatch: ~A" n))
                         (loop (expval->cdr list)
                               (extend-nl-env (expval->car list) e)
                               (- n 1))))]
