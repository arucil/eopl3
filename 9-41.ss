...
(define (init-class-decl! c-decl)
  (cases class-decl c-decl
    ...
    [an-interface-decl (i-name am-decls)
                       1]
    ))
  ...
  (cases class-decl c-decl
    ...
    [a-class-decl (c-name s-name i-names f-types f-names m-decls)
                  (let* ([s-class (lookup-static-class s-name)]
                    ...
                    (check-no-dups! f-names1 c-name)
                    (check-field-types! (map cons
                                             (static-class->field-names s-class)
                                             (static-class->field-types s-class))
                                        (map cons f-names f-types)
                                        c-name)
...
(define (check-field-types! s-f-tenv new-f-tenv c-name)
  (for-each (lambda (new-f-binding)
              (let* ([new-f-name (car new-f-binding)]
                     [s-p (assv new-f-name s-f-tenv)])
                (when s-p
                    ;; type of field in super class must be subtype of that of the same field in subclass 
                    (check-is-subtype! (cdr s-p)
                                       (cdr new-f-binding)
                                       new-f-name))))
            new-f-tenv))
...
(define (type-of exp tenv)
  (cases expression exp
    ...
    [fieldref-exp (exp1 f-name)
                  (let ([ty (type-of exp1 tenv)])
                    (if (and (class-type? ty)
                             (is-static-class? (type->class-name ty)))
                        (let* ([sc (lookup-static-class
                                   (type->class-name ty))]
                               [loc (location f-name (static-class->field-names sc))])
                          (if loc
                              (list-ref (static-class->field-types sc) loc)
                              (eopl:error 'type-of
                                          "field ~A not found in class ~A"
                                          f-name (type->class-name ty))))
                        (eopl:error 'type-of
                                    "bad type ~A to fieldref ~A in ~A"
                                    (format-type ty) f-name exp)))]
    [fieldset-exp (exp1 f-name exp2)
                  (let ([ty (type-of exp1 tenv)])
                    (if (and (class-type? ty)
                             (is-static-class? (type->class-name ty)))
                        (let* ([sc (lookup-static-class
                               ...
                               [loc (location f-name (static-class->field-names sc))])
                          (if loc
                              (check-is-subtype! (type-of exp2 tenv)
                                                 (list-ref (static-class->field-types sc) loc)
                                                 exp)
                              (eopl:error 'type-of
                                          "field ~A not found in class ~A"
...
class c1 extends object
 ...
 method listof int get() list(x, y)
class c2 extends c1
 field int y
let o1 = new c1()
    o2 = new c2()
