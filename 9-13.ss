...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (method-decl
     (final? "method" accessibility identifier "(" (separated-list identifier ",") ")" expression)
     a-method-decl)
                  ...
                  (let ([f-env (append-field-env (class->field-env (lookup-class s-name))
                    ...
                    (add-to-class-env! c-name
                                       (a-class s-name f-env
                                                (extend-method-env-with-decls
                                                  m-decls
                                                  c-name
                                                  (field-env->field-names f-env)
...
(define (extend-method-env-with-decls m-decls c-name f-names s-m-env)
  (if (null? m-decls)
      s-m-env
      (cases method-decl (car m-decls)
        [a-method-decl (fin? acc m-name vars body)
                       (let ([p (assv m-name s-m-env)])
                         (when p
                           (cases method (cdr p)
                             [a-method (a b c d e fin?)
                                       (when fin?
                                         (eopl:error 'value-of
                                                     "attempt to override final method ~A in class ~A"
                                                     m-name c-name))])))
                       (cons (cons m-name
                                   (a-method vars body c-name f-names acc
                                             (cases final? fin?
                                               [non-final () #f]
                                               [final () #t])))
                             (extend-method-env-with-decls (cdr m-decls)
                                                           c-name
                                                           f-names
                                                           s-m-env))])))
...
(define (method->class-name m)
  (cases method m
    [a-method (vars body c-name f-names acc f?)
              c-name]))
...
(define (apply-method c-name m-name self args env)
  (cases method (find-method c-name m-name)
    [a-method (vars body class-name field-names acc fin?)
              (let ([self0 (current-self env)])
                (unless (or
                         (and
                          (private-acc? acc)
