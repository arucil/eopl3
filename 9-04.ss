...
(define (find-method c name)
  (let ([p (assq name (class->method-env c))])
    (if p
        (cdr p)
  ...
  (cases expression exp
    ...
    [method-call-exp (obj-exp m-name rands)
                     (let ([obj (value-of obj-exp env)])
                       (apply-method
                        (find-method (object->class obj)
                                     m-name)
                        obj
                        (values-of-exps rands env)))]
    [super-call-exp (m-name rands)
                    (apply-method (find-method (lookup-class (apply-env env '%super)) m-name)
                                  (apply-env env '%self)
                                  (values-of-exps rands env))]
    [new-object-exp (c-name rands)
                    (let ([obj (new-object (lookup-class c-name))])
                      (apply-method (find-method (lookup-class c-name) 'initialize)
                                    obj
                                    (values-of-exps rands env))
                      obj)]
    ))
