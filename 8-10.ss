...
(define the-grammar
  '((program ((arbno module-definition) module-dependency expression) a-program)
    ...
    (module-definition
     ("module" identifier "interface" interface "body" module-dependency module-body)
     a-module-definition)
    ...
    (module-dependency
     ("import" identifier (arbno "," identifier))
     mod-deps)
...
(define-datatype module-thunk module-thunk?
  [a-module-thunk
   (m-body module-body?)
   (env environment?)])
...
(define (lookup-qualified-var-in-env m-name var-name env)
  (cases typed-module (deref (lookup-module-name-in-env m-name env))
...
(define (run prog)
  (let ([prg (scan&parse prog)])
    (type-of-program prg)
    (cases program prg
      [a-program (defns md exp)
                 (init-store)
                 (let ([env (add-module-defns-to-env defns (empty-env))])
                   (eval-modules md env)
                   (value-of exp env))])))
...
(define (add-module-defns-to-env defns env)
  (if (null? defns)
      ...
      (cases module-definition (car defns)
        [a-module-definition (m-name iface md m-body)
                             (eval-modules md env)
                             (add-module-defns-to-env (cdr defns)
                                                      (extend-env-with-module m-name
                                                                              (newref (a-module-thunk m-body env))
                                                                              env))])))

(define (eval-modules mod-dep env)
  (cases module-dependency mod-dep
    [no-dep () 1]
    [mod-deps (var1 vars)
              (let loop ([vars (cons var1 vars)])
                (unless (null? vars)
                  (let* ([ref (lookup-module-name-in-env (car vars) env)]
                         [m-thunk (deref ref)])
                    (when (module-thunk? m-thunk)
                      (cases module-thunk m-thunk
                        [a-module-thunk (m-body env1)
                                        (setref! ref (value-of-module-body m-body env1))]))
                    (loop (cdr vars)))))]))
...
(define (type-of-program prg)
  (cases program prg
    [a-program (defns mod-dep exp1)
               (let ([mod-tenv (add-module-defns-to-tenv defns (empty-tenv))])
                 (cases module-dependency mod-dep
                   [no-dep ()
                           (type-of exp1 (empty-tenv))]
                   [mod-deps (var1 vars)
                             (type-of exp1 (build-module-tenv (cons var1 vars) mod-tenv))]))]))

(define (build-module-tenv vars tenv)
  (let loop ([vars vars]
             [tenv1 (empty-tenv)])
    (if (null? vars)
        tenv1
        (loop (cdr vars)
              (extend-tenv-with-module (car vars)
                                       (lookup-module-name-in-tenv (car vars)
                                                                   tenv)
                                       tenv1)))))
...
(define (add-module-defns-to-tenv defns tenv)
  (if (null? defns)
      tenv
      (cases module-definition (car defns)
        [a-module-definition (m-name expected-iface mod-dep m-body)
                             (let* ([tenv1 (cases module-dependency mod-dep
                                             [no-dep ()
                                                     (empty-tenv)]
                                             [mod-deps (var1 vars)
                                                       (build-module-tenv (cons var1 vars)
                                                                          tenv)])]
                                    [actual-iface (interface-of m-body tenv1)])
