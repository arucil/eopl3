...
(define the-grammar
  '((program ((arbno mod-iface-definition) expression) a-program)
...
(define (apply-tenv tenv svar)
  (cases tenvironment tenv
    ...
    [extend-tenv-with-type (name i e)
                           (apply-tenv e svar)]
    [extend-tenv-with-interface (a b e)
                                (apply-tenv e svar)]
    ))

(define (lookup-qualified-var-in-tenv m-name var-name tenv)
  (let rec ([iface (lookup-module-name-in-tenv m-name tenv)])
    (cases interface iface
      ...
      [proc-iface (name iface1 iface2)
                  (eopl:error 'type-of
                              ...
                              m-name var-name)]
      [named-iface (name)
                   (rec (lookup-interface-in-tenv name tenv))]
      )))

(define (lookup-interface-in-tenv i-name tenv)
  (cases tenvironment tenv
    [empty-tenv ()
                (eopl:error 'type-of "interface ~s is not bound" i-name)]
    [extend-tenv (var ty env)
                 (lookup-interface-in-tenv i-name env)]
    [extend-tenv-with-module (name i e)
                             (lookup-interface-in-tenv i-name e)]
    [extend-tenv-with-type (name i e)
                           (lookup-interface-in-tenv i-name e)]
    [extend-tenv-with-interface (name iface e)
                                (if (eqv? name i-name)
                                    iface
                                    (lookup-interface-in-tenv i-name e))]))
...
(define (add-module-defns-to-env defns env)
  (if (null? defns)
      env
      (cases mod-iface-definition (car defns)
        [mod-defn (m-name iface m-body)
                             (add-module-defns-to-env
                              ...
                              (extend-env-with-module
                               ...
                               env))]
        [iface-defn (a b)
                    (add-module-defns-to-env (cdr defns) env)])))
...
(define (add-module-defns-to-tenv defns tenv)
  (if (null? defns)
      tenv
      (cases mod-iface-definition (car defns)
        [mod-defn (m-name expected-iface m-body)
...
(define (interface-of m-body tenv)
  (cases module-body m-body
    ...
    [var-module-body (m-name)
                     (lookup-module-name-in-tenv m-name tenv)]
    [proc-module-body (rand-name rand-iface m-body)
                      (proc-iface rand-name rand-iface
                                  (interface-of
                                   m-body
                                   (extend-tenv-with-module
                                    rand-name
    ...
    [app-module-body (rator-id rand-id)
                     (let ([rator-iface (lookup-module-name-in-tenv rator-id tenv)]
                           [rand-iface (lookup-module-name-in-tenv rand-id tenv)])
                       (let rec ([rator-iface rator-iface])
                         (cases interface rator-iface
                           ...
                           [proc-iface (param-name param-iface result-iface)
                                       (if (<:-iface rand-iface param-iface tenv)
                                           (rename-in-iface result-iface param-name rand-id tenv)
                                           (eopl:error 'type-of
...
(define (rename-in-iface iface var0 var1 tenv)
  (let rec ([iface iface])
    (cases interface iface
      [simple-iface (decls)
...
(define (expand-iface m-name iface tenv)
  (cases interface iface
    ...
    [proc-iface (name aiface riface)
                iface]
    [named-iface (name)
                 (expand-iface m-name (lookup-interface-in-tenv name tenv) tenv)]
    ))
                ...
                (let rec ([iface2 iface2])
                  (cases interface iface2
                    ...
                    [proc-iface (param-name2 param-iface2 result-iface2)
                                (let ([new-name (new-module-name param-name1)])
                                  (and
                                   ...
                                   (<:-iface (rename-in-iface result-iface1 param-name1 new-name tenv)
                                             (rename-in-iface result-iface2 param-name2 new-name tenv)
                    ...
                    [named-iface (name)
                                 (rec (lookup-interface-in-tenv name tenv))]))]
    [named-iface (name)
                 (<:-iface (lookup-interface-in-tenv name tenv)
                           iface2 tenv)]
    ))
