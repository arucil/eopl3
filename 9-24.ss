...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (expression
     ("%named-send" identifier expression number "(" (arbno expression) ")")
     nl-named-method-call-exp)
...
(define (init-class-env! c-decls)
  (set! the-class-env (list (cons 'object (a-class #f '() (make-vector 0)))))
...
(define (find-method c-name name)
  (let ([m-env (class->method-env (lookup-class c-name))])
    (let loop ([i 0])
      (if (= (vector-length m-env)
             i)
          (eopl:error 'find-method
                      "method ~A not found in class ~A"
                      name c-name)
          (let ([p (vector-ref m-env i)])
            (if (eqv? (car p) name)
                (cons (cdr p) i)
                (loop (+ i 1))))))))

(define (locate-method c-name m-index)
  (cdr (vector-ref (class->method-env (lookup-class c-name))
                   m-index)))

(define (method-decls->method-env m-decls s-name f-names)
  (let ([m-env (make-vector (length m-decls))])
    (let loop ([i 0]
               [m-decls m-decls])
      (unless (null? m-decls)
          (cases method-decl (translation-of-method-decl (car m-decls))
...
(define (merge-method-envs s-m-env new-m-env)
  (let* ([len0 (vector-length new-m-env)]
         [len1 (vector-length s-m-env)]
         [new-env (make-vector (+ len0 len1))])
    (let loop ([i (- len0 1)])
      (unless (negative? i)
        (vector-set! new-env i
                     (vector-ref new-m-env i))
        (loop (- i 1))))
    (let loop ([i (- len1 1)]
               [j (- (+ len1 len0) 1)])
      (unless (negative? i)
        (vector-set! new-env j
                     (vector-ref s-m-env i))
        (loop (- i 1)
              (- j 1))))
    new-env))
...
(define (named-field-ref obj c-name f-name)
  (cases object obj
    [an-object (cn fs)
  ...
  (cases expression exp
    ...
    [method-call-exp (obj-exp m-name rands)
                     (let ([obj (value-of obj-exp env)])
                       (apply-method
                        (car (find-method (object->class-name obj)
                                          m-name))
                        obj
                        (values-of-exps rands env)))]
    [super-call-exp (m-name rands)
                    (apply-method (car (find-method (apply-env env '%super) m-name))
                                  (apply-env env '%self)
                                  (values-of-exps rands env))]
    [new-object-exp (c-name rands)
                    (let ([obj (new-object c-name)])
                      (apply-method (car (find-method c-name 'initialize))
                                    obj
                                    (values-of-exps rands env))
                      obj)]
    [nl-named-method-call-exp (c-name exp1 m-index rands)
                           (let ([obj (value-of exp1 env)])
                             (ensure-instance-of obj c-name)
                             (apply-method
                              (locate-method c-name m-index)
                              obj
                              (values-of-exps rands env)))]
    [named-method-call-exp (c-name exp1 m-name rands)
                           (eopl:error 'value-of
                                       "unreachable ~A"
                                       exp)]
    [named-fieldref-exp (c-name exp1 f-name)
                        (let ([obj (value-of exp1 env)])
                          (ensure-instance-of obj c-name)
                          (deref (named-field-ref obj c-name f-name)))]
    [named-fieldset-exp (c-name exp1 f-name exp2)
                        (let ([obj (value-of exp1 env)])
                          (ensure-instance-of obj c-name)
                          (setref! (named-field-ref obj c-name f-name)
                                   (value-of exp2 env)))]
    ))

(define (ensure-instance-of obj c-name)
  (let loop ([cn (object->class-name obj)])
    (unless (eqv? cn c-name)
      (let ([s-name (class->super-name (lookup-class cn))])
        (if s-name
            (loop s-name)
            (eopl:error 'value-of
                        "object ~A is not an instance of ~A"
                        obj c-name))))))
...
(define (translation-of exp)
  (cases expression exp
    ...
    [super-call-exp (m-name rands)
                    (super-call-exp m-name
                                    (map translation-of rands))]
    [new-object-exp (c-name rands)
                    (new-object-exp c-name
                                    (map translation-of rands))]
    [named-fieldref-exp (c-name exp1 f-name)
                        (named-fieldref-exp c-name
                                            (translation-of exp1)
                                            f-name)]
    [named-fieldset-exp (c-name exp1 f-name exp2)
                        (named-fieldset-exp c-name
                                            (translation-of exp1)
                                            f-name
                                            (translation-of exp2))]
    [named-method-call-exp (c-name exp1 m-name rands)
                           (nl-named-method-call-exp c-name
                                                     (translation-of exp1)
                                                     (cdr (find-method c-name m-name))
                                                     (map translation-of rands))]
    [else
...
(test (make-list
       (list
        (num-val 11) (num-val 12) (num-val 13)
        (num-val 21) (num-val 22) (num-val 23)
        (num-val 1) (num-val 200) (num-val 3000) (num-val 4)
        (num-val 10) (num-val 2000) (num-val 3000) (num-val 4)
        (num-val 10) (num-val 20000) (num-val 30000) (num-val 4)))
      "
class c1 extends object
 field x
 field y
...
in
begin
 named-fieldset c1 o y = 200;
 named-fieldset c2 o y = 2000;
 named-fieldset c3 o y = 20000;
 named-fieldset c1 o z = 300;
 named-fieldset c2 o z = 3000;
 named-fieldset c3 o z = 30000;
 list(named-send c1 o m1(),
      named-send c2 o m1(),
      named-send c3 o m1(),
      named-send c1 o m2(),
      named-send c2 o m2(),
      named-send c3 o m2(),
      named-fieldref c1 o x,
      named-fieldref c1 o y,
      named-fieldref c1 o z,
      named-fieldref c1 o w,
      named-fieldref c2 o x,
      named-fieldref c2 o y,
      named-fieldref c2 o z,
      named-fieldref c2 o w,
      named-fieldref c3 o x,
      named-fieldref c3 o y,
      named-fieldref c3 o z,
      named-fieldref c3 o w);
end")
