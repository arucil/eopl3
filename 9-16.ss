  ...
  (map (lambda (m-decl)
         (cases method-decl m-decl
           [a-method-decl (m-name vars body)
                          (cons (make-method-name m-name vars)
                                (a-method vars body s-name f-names))]))
       m-decls))

(define (make-method-name m-name ls)
  (string->symbol
   (string-append
...
(define (value-of exp env)
  (cases expression exp
    ...
    [super-call-exp (m-name rands)
                    (apply-method (find-method (apply-env env '%super)
                                               (make-method-name m-name rands))
                                  (apply-env env '%self)
                                  (values-of-exps rands env))]
    [new-object-exp (c-name rands)
                    (let ([obj (new-object c-name)])
                      (apply-method (find-method c-name
                                                 (make-method-name 'initialize rands))
                                    obj
                                    (values-of-exps rands env))
                      obj)]
    ))
