  ...
  (if (time-expired?)
      (begin
        (place-on-ready-queue!
         (cont-thread cont val (fork-store) (get-thread-id) (get-thread-parent-id))
         the-max-time-slice)
        (run-next-thread))
      (begin
        (decrement-timer!)
...
(define (thread->id th)
  (cases thread th
    [cont-thread (c v s i p)
                 i]
    [new-thread (pr v c s i p)
                i]))

(define (run-thread th)
  (cases thread th
    [new-thread (proc vals cont store id pid)
                (set! current-thread-id id)
                (set! thread-parent-id pid)
                (set-store! store)
                (apply-procedure/k proc vals cont)]
    [cont-thread (cont val store id pid)
                 (set! current-thread-id id)
                 (set! thread-parent-id pid)
                 (set-store! store)
                 (apply-cont cont val)]))
...
(define (yield-current-thread cont)
  (place-on-ready-queue! (cont-thread cont
                                      (num-val 99)
                                      (fork-store)
                                      (get-thread-id)
                                      (get-thread-parent-id))
    ...
    (filter-blocking-queue
     (lambda (ti)
       (cases thread (get-thread ti)
         [cont-thread (cont1 val1 store id pid)
                          (if (and (not sent)
                                   (= id
                                      send-id))
                              (begin
                                (set! sent #t)
                                (place-on-ready-queue!
                                 (cont-thread cont1 val store id pid)
                                 (get-time ti))
                                #f)
                              #t)]
         [else
