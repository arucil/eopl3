                  ...
                  (let ([f-names (append-field-names (class->field-names (lookup-class s-name))
                    ...
                    (add-to-class-env! c-name
                                       (a-class s-name f-names
                                                (let ([a (merge-method-envs
                                                 (class->method-env (lookup-class s-name))
                                                 (method-decls->method-env
                                                  m-decls s-name f-names))])
                                                  ;(eopl:printf "class ~A:~%" c-name)
                                                  ;(print-method-env a)
                                                  ;(display "----\n")
                                                  a))))]))
...
(define (merge-method-envs s-m-env new-m-env)
  (if (null? new-m-env)
      s-m-env
      (let* ([m-pair (car new-m-env)]
             [smenv1 (subst s-m-env
                            (car m-pair)
                            m-pair
                            (lambda (name pair)
                              (eqv? name (car pair))))])
        (if smenv1
            (merge-method-envs smenv1
                               (cdr new-m-env))
            (cons m-pair
                  (merge-method-envs s-m-env (cdr new-m-env)))))))

(define (print-method-env m-env)
  (unless (null? m-env)
      (begin
        (eopl:printf "method ~A~%" (caar m-env))
        (print-method-env (cdr m-env)))))
