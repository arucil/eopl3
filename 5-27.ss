...
;;  foo(proc1, exp1)  =>  +( (proc1 -(exp1, 1)), exp1)
...
(define (apply-cont)
  (cases continuation cont
    ...
    [foo-rator-cont (exp2 senv cont1)
                    (set! cont (foo-rand-cont val cont1))
                    (set! exp exp2)
                    (set! env senv)
                    (value-of/k)]
    [foo-rand-cont (val1 cont1)
                   (let ([old-val val])
                     (set! cont (foo-call-cont old-val cont1))
                     (set! val (num-val (-
                                         (expval->num old-val)
                                         1)))
                     (set! proc1 (expval->proc val1))
                     (apply-procedure/k))]
    [foo-call-cont (val1 cont1)
                   (set! cont cont1)
                   (set! val (num-val (+
                                       (expval->num val)
                                       (expval->num val1))))
...
(define (value-of/k)
  (cases expression exp
    ...
    [foo-exp (exp1 exp2)
             (set! exp exp1)
             (set! cont (foo-rator-cont exp2 env cont))
             (value-of/k)]
  ))