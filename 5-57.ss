    ...
    (letrec ([disp (lambda (v need-par)
                     (cases expval v
                       ...
                       [null-val ()
                                 (display "()")]
                       [mutex-val (m)
                                  (eopl:printf "#mutex(~A)" (mutex-closed? m))]
                       [sem-val (s)
                                (eopl:printf "#semaphore(~A)" (semaphore-num s))]
                       [pair-val (pair)
                                 (when need-par
                                   (display "("))
                                 (disp (car pair) #t)
...
(define (wait-cont cont)
  (lambda (val)
    (cases expval val
      ...
      [sem-val (sem)
               (wait-for-semaphore sem
                                   (cont-thread cont
                                              (num-val 1)
                                              (get-thread-id)
                                              (get-thread-parent-id)))]
      [else
       (eopl:error 'wait-cont "expval ~A is not mutex or semaphore" val)])))
      ...
      [sem-val (sem)
               (signal-semaphore sem
                                 (cont-thread cont
                                              ...
                                              (get-thread-parent-id)))]
      [else
       (eopl:error 'signal-cont "expval ~A is not mutex or semaphore" val)])))

(define (sem-cont cont)
  (lambda (val)
    (apply-cont cont (sem-val (new-semaphore (expval->num val))))))
    ...
    (let* ([id (expval->num val)]
      ...
      (foreach-env env
                   (lambda (var val)
                     (cases expval val
                       [mutex-val (m)
                                  (filter-mutex-queue m kill-f)]
                       [sem-val (sem)
                                (filter-semaphore-queue sem kill-f)]
                       [else 1])))
...
(define (wait-for-semaphore sem th)
  (cases semaphore sem
    [a-semaphore (ref-num ref-queue)
                 (let ([num (deref ref-num)])
                   (if (positive? num)
                       (begin
                         (setref! ref-num (- num 1))
                         (run-thread th))
                       (begin
                         (setref! ref-queue
                                  (enqueue (deref ref-queue)
                                           (thread-info th the-time-remaining)))
                         (run-next-thread))))]))
                           ...
                           (dequeue queue
                                    (lambda (ti1 q1)
                                      (place-on-ready-queue! (get-thread ti1)
                                                             (get-time ti1))
                                      (setref! ref-queue q1))))
                       (setref! ref-num (+ 1 num)))
                   (run-thread th))]))

(define (semaphore-num sem)
  (cases semaphore sem
    [a-semaphore (i q)
                 (deref i)]))

(define (filter-semaphore-queue sem f)
  (cases semaphore sem
    [a-semaphore (ref-num ref-queue)
             (setref! ref-queue (filter-queue (deref ref-queue) f))]))
