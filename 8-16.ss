             ...
             (let loop ([vars vars]
               ...
               (if (null? vars)
                   ...
                   (loop (cdr vars)
                         (cdr exps)
                         (extend-tenv (car vars)
                                      (expand-type (type-of (car exps) tenv) tenv)
                                      tenv1))))]
...
body [
 type table = (int -> int)
 empty = proc (x: int) 0
 add-to-table = proc(var: int, val: int, t: table)
                 proc(x: int)
                  if zero?(-(x, var))
                  then val
                  else (t x)
 lookup-in-table = proc(var:int, t:table)
                    (t var)
]
let empty = from table take empty
    add-binding = from table take add-to-table
    lookup = from table take lookup-in-table
in let t1 = (add-binding 3 300
             (add-binding 4 400
              (add-binding 3 600
               empty)))
in -((lookup 4 t1), (lookup 3 t1))
"))