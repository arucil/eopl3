...
(define (extend-senv-letrec var env)
  (cons (cons var #t) env))
...
(define (apply-senv env var)
  (cond
    ...
    [(eqv? (caar env) var)
     (cons 0 (cdar env))]
    [else
     (let ([result (apply-senv (cdr env) var)])
       (cons (+ 1 (car result))
             (cdr result)))]))
...
(define (translation-of exp senv)
  (cases expression exp
    ...
    [letrec-exp (var arg exp body)
                (nl-letrec-exp (translation-of exp
                                               (extend-senv arg
...
(define (value-of exp env)
  (cases expression exp
    ...
    [nl-letrec-exp (exp body)
                   (value-of body
                             (extend-nl-env (proc-val (procedure exp env)) env))]

    [nl-letrec-var-exp (n)
                       (let ([new-nl-env (shrink-nl-env env n)])
                         (cases proc (expval->proc (apply-nl-env new-nl-env 0))
                           [procedure (exp env)
                                      (proc-val (procedure exp new-nl-env))]
                           [else
                            (eopl:error 'value-of "unreachable")]))]
