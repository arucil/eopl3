...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (expression
     ("%send" (arbno identifier) "," expression "(" (arbno identifier) ":" (arbno expression) ")")
     static-method-call)
...
(define (value-of exp env)
  (cases expression exp
    ...
    [super-call-exp (m-name rands)
                    (eopl:error 'value-of
                                "invalid super call ~A"
                                m-name)]
    [static-method-call (vars body f-names rands)
                        (apply-static-method vars body f-names
                                             (deref (apply-env env '%self))
                                             rands)]
...
(define (apply-method m self args)
  (cases method m
    [a-method (vars body field-names)
              (apply-static-method vars body field-names self args)]))

(define (apply-static-method vars body f-names self args)
  (value-of body
            (extend-env* vars
                         (map newref args)
                         (extend-env
...
(define (translation-of exp s-name)
  (cases expression exp
    [const-exp (num)
      exp]
    [diff-exp (exp1 exp2)
      (diff-exp (translation-of exp1 s-name)
                (translation-of exp2 s-name))]
    [plus-exp (exp1 exp2)
      (plus-exp (translation-of exp1 s-name)
                (translation-of exp2 s-name))]
    [var-exp (var)
      exp]
    [zero?-exp (exp1)
      (zero?-exp (translation-of exp1 s-name))]
    [if-exp (exp1 exp2 exp3)
      (if-exp (translation-of exp1 s-name)
              (translation-of exp2 s-name)
              (translation-of exp3 s-name))]
    [let-exp (vars exps body)
             (let-exp vars
                      (translations-of-exps exps s-name)
                      (translation-of body s-name))]
    [letrec-exp (p-names b-varss bodies exp1)
                (letrec-exp p-names b-varss
                            (translations-of-exps bodies s-name)
                            (translation-of exp1 s-name))]
    [proc-exp (vars exp1)
              (proc-exp vars
                        (translation-of exp1 s-name))]

    [call-exp (exp1 exps)
              (call-exp (translation-of exp1 s-name)
                        (translations-of-exps exps s-name))]
    ...
    [begin-exp (exp1 exps)
               (begin-exp (translation-of exp1 s-name)
                          (translations-of-exps exps s-name))]

    [print-exp (exp1)
               (print-exp (translation-of exp1 s-name))]
    [null-exp ()
              exp]
    [null?-exp (exp1)
               (null?-exp (translation-of exp1 s-name))]
    [car-exp (exp1)
             (car-exp (translation-of exp1 s-name))]
    [cdr-exp (exp1)
             (cdr-exp (translation-of exp1 s-name))]
    [cons-exp (exp1 exp2)
              (cons-exp (translation-of exp1 s-name)
                        (translation-of exp2 s-name))]
    [list-exp (exps)
              (list-exp (translations-of-exps exps s-name))]

    [self-exp ()
              exp]
    [method-call-exp (obj-exp m-name rands)
                     (method-call-exp (translation-of obj-exp s-name)
                                      m-name
                                      (translations-of-exps rands s-name))]
    [super-call-exp (m-name rands)
                    (cases method (find-method s-name m-name)
                      [a-method (vars body f-names)
                                (static-method-call
                                 vars body f-names
                                 (translations-of-exps rands s-name))])]
    [new-object-exp (c-name rands)
...
(define (translations-of-exps exps s-name)
  (map (lambda (x)
         (translation-of x s-name))
       exps))
