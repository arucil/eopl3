                      ...
                      (if n
                          (newref
                           (proc-val (procedure
                                      ...
                                      env)))
                          (apply-env e svar)))]
    [extend-env-with-self-and-host (self host e)
                                    (cond
                                      [(eqv? svar '%self)
                                       self]
                                      [(eqv? svar '%host)
...
(define (current-self env)
  (cases environment env
    ...
    [extend-env-rec (a b c e)
                    (current-self e)]
    [extend-env-with-self-and-host (self b e)
                                    self]))
  ...
  (map (lambda (m-decl)
         (cases method-decl m-decl
           [a-method-decl (m-name vars body)
                          (cons m-name
                                (a-method vars body c-name f-names))]))
       m-decls))
  ...
  (cases expression exp
    ...
    [method-call-exp (obj-exp m-name rands)
                     (let ([obj (value-of obj-exp env)]
                           [self0 (current-self env)])
                       (apply-method
                        (find-method (if (and self0
                                              (equal? self0 obj))
                                         (apply-env env '%host)
    ...
    [super-call-exp (m-name rands)
                    (apply-method (find-method (class->super-name (lookup-class (apply-env env '%host))) m-name)
                                  (apply-env env '%self)
                                  (values-of-exps rands env))]
