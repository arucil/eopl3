...
(define (cps-of-program prog)
  (cases program prog
    [a-program (exp1)
               (cps-a-program
                (cps-of-exp/ctx exp1 #f
                             (lambda (v)
                               (simple-exp->exp v))))]))

(define (cps-of-exps exps k-err builder)
...
(define (cps-of-exp/ctx exp k-err context)
...
(define (cps-of-exp exp k-exp k-err)
  (cases expression exp
    [zero?-exp (exp1)
               (cps-of-exp/ctx exp1 k-err
                               (lambda (simple)
            ...
            (cps-of-exp/ctx exp1 k-err
                            (lambda (simple)
                              (cases cps-simple-exp k-exp
                                [cps-var-exp (v)
                                             (cps-if-exp simple
                                                         (cps-of-exp exp2 k-exp k-err)
                                                         (cps-of-exp exp3 k-exp k-err))]
                                [else
                                 (let ([k-var (new-identifier 'k%)])
                                   (cps-let-exp k-var
                                                ...
                                                (cps-if-exp
                                                 simple
                                                 (cps-of-exp exp2
                                                             (cps-var-exp k-var) k-err)
                                                 (cps-of-exp exp3
                                                             (cps-var-exp k-var) k-err))))])))]
    [let-exp (var exp1 body)
             (cps-of-exp exp1
                         (cps-proc-exp (list var)
                                       (cps-of-exp body k-exp k-err))
                         k-err)]
    [letrec-exp (p-names b-varss p-bodies body)
                (cps-letrec-exp p-names
                                (map (lambda (b-vars) (append b-vars (list 'k%00)))
                                     b-varss)
                                (map (lambda (p-body) (cps-of-exp p-body (cps-var-exp 'k%00) #f))
                                     p-bodies)
                                (cps-of-exp body k-exp k-err))]
    [call-exp (rator rands)
              (cps-of-exp/ctx rator k-err
                              (lambda (simple1)
                                (cps-of-exps rands k-err
                                             (lambda (simples)
    ...
    [setref-exp (exp1 exp2)
                (cps-of-exps (list exp1 exp2) k-err
                               (lambda (simples)
    ...
    [letcc-exp (var exp1)
               (cps-let-exp var k-exp
                            (cps-of-exp exp1 (cps-var-exp var) k-err))]
    [throw-exp (exp1 exp2)
               (cps-of-exps (list exp1 exp2) k-err
                            (lambda (simples)
    ...
    [raise-exp (exp1)
               (cps-of-exp/ctx exp1 k-err
                               (lambda (simple1)
                                 (if k-err
                                     (make-send-to-cont k-err simple1)
                                     (eopl:error 'cps-of-exp "uncaught exception ~A" exp1))))]
    [else
     (make-send-to-cont k-exp
  ...
  (cases expression exp
    ...
    [proc-exp (vars body)
              (cps-proc-exp (append vars (list 'k%00))
                            (cps-of-exp body (cps-var-exp 'k%00) #f))]
    [else 'error]))
