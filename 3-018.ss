...
(define (value-of exp env)
  (cases expression exp
    ...
    [let*-exp (var-list exp-list body)
      (letrec ([rec (lambda (var-list exp-list env)
                      (if (null? var-list)
                          env
                          (rec (cdr var-list)
                            (cdr exp-list)
    ...
    [unpack-exp (var-list exp body)
      (letrec ([rec (lambda (var-list val-list e)
                      (if (null? var-list)
                          (if (not (expval-null? val-list))
                              (eopl:error 'value-of "unpack operand mismatch: ~A" exp)
                              e)
                          (rec (cdr var-list)
                            (cdr (expval->cons val-list))
                            (extend-env (car var-list)
                                        (car (expval->cons val-list))
                                        e))))])
        (value-of body (rec var-list (value-of exp env) env)))]
